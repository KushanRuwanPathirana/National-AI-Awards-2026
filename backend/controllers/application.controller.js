...
const getPublicUploadPath = (file) =>
  path.posix.join("uploads", "documents", file.filename);

const hasApplicationDeadlinePassed = () =>
  Date.now() >= new Date(APPLICATION_DEADLINE.CLOSES_AT).getTime();
    ...
    application.documents.forEach((doc) => {
      ...
      if (diskPath && fs.existsSync(diskPath)) {
        try {
          ...
          logger.error(
            `Failed to delete document file from disk: ${e.message}`,
          );
        }
      }
    });
...
const createApplication = async (req, res, next) => {
  try {
    ...
    await createAuditLog({
      ...
      targetId: application._id,
      description: `New draft: ${projectTitle || organisationName || organizationName || "Untitled application"}`,
      req,
    });
  ...
  try {
    ...
    if (!application.category || !application.projectTitle) {
      return errorResponse(res, {
        statusCode: 400,
        message:
          "Please select a category and enter a project title before submitting.",
      });
    }
    ...
    if (
      !application.verificationConsent ||
      !application.promotionalConsent ||
      !application.submissionFeeAcknowledged
    ) {
      return errorResponse(res, {
        statusCode: 400,
        message:
          "Please complete all required declarations and consents before submitting.",
      });
    }
    if (!application.problemStatement || !application.solution) {
      return errorResponse(res, {
        statusCode: 400,
        message: "Please complete all required form fields before submitting.",
      });
    }
    if (
      ...
      !application.categoryEligibilityConfirmed
    ) {
      return errorResponse(res, {
        statusCode: 400,
        message:
          "Please complete applicant details and category eligibility confirmation before submitting.",
      });
    }

    // Validate payment if it's not a free category
    const isFreeCategory =
      application.category?.name === "University AI Innovation";
    if (!isFreeCategory) {
      ...
      if (
        ...
        (!application.paymentSlip || !application.paymentSlip.filePath)
      ) {
        return errorResponse(res, {
          statusCode: 400,
          message: "Please upload your bank transfer slip before submitting.",
        });
      }
      if (
        application.paymentMethod === "online" &&
        !application.onlinePaymentSimulated
      ) {
        return errorResponse(res, {
          statusCode: 400,
          message:
            "Please complete the online payment simulation before submitting.",
        });
      }
    }
    ...
    await createAuditLog({
      ...
      performedBy: req.user._id,
      targetId: application._id,
      description: `Submitted: ${application.projectTitle}`,
      req,
    });
    ...
    try {
      ...
      logger.error(`Submit email error: ${e.message}`);
    }
...
const changeApplicationStatus = async (req, res, next) => {
  try {
    ...
    if (!allowed.includes(status)) {
      logger.warn(
        `Admin ${req.user._id} overriding status transition: from "${application.status}" to "${status}". Allowed: ${allowed.join(", ") || "none"}`,
      );
    }
...
const assignJudges = async (req, res, next) => {
  try {
    ...
    if (!judgeIds || !Array.isArray(judgeIds)) {
      return errorResponse(res, {
        statusCode: 400,
        message: "judgeIds array is required.",
      });
    }
    ...
    await createAuditLog({
      ...
      performedBy: req.user._id,
      targetId: application._id,
      description: `Assigned ${judgeIds.length} judge(s)`,
      req,
    });

    const updated = await Application.findById(id).populate(
      "assignedJudges",
...
const uploadDocuments = async (req, res, next) => {
  try {
    ...
    if (application.documents.length + req.files.length > 2) {
      return errorResponse(res, {
        statusCode: 400,
        message: "Maximum 2 PDF documents can be uploaded per application.",
      });
    }
    ...
    return successResponse(res, {
      message: `${newDocs.length} document(s) uploaded.`,
...
const deleteDocument = async (req, res, next) => {
  try {
    ...
    if (application.status !== APPLICATION_STATUS.DRAFT) {
      return errorResponse(res, {
        statusCode: 400,
        message: "Documents can only be removed from draft applications.",
      });
    }
...
const uploadPaymentSlip = async (req, res, next) => {
  try {
    ...
    if (application.status !== APPLICATION_STATUS.DRAFT) {
      return errorResponse(res, {
        statusCode: 400,
        message: "Payment slip can only be uploaded to draft applications.",
      });
    }
    ...
    if (application.paymentSlip && application.paymentSlip.filePath) {
      ...
      if (diskPath && fs.existsSync(diskPath)) {
        try {
          ...
          logger.error(
            `Failed to delete old payment slip from disk: ${e.message}`,
          );
        }
      }
    }
    ...
    return successResponse(res, {
      ...
      data: { paymentSlip: application.paymentSlip },
    });
  } catch (error) {
    next(error);
  }
...
const deletePaymentSlip = async (req, res, next) => {
  try {
    ...
    if (application.status !== APPLICATION_STATUS.DRAFT) {
      return errorResponse(res, {
        statusCode: 400,
        message: "Payment slip can only be deleted from draft applications.",
      });
    }
    ...
    return successResponse(res, {
      message: "Payment slip removed successfully.",
    });
  } catch (error) {
    next(error);
  }
...
const reviewEligibility = async (req, res, next) => {
  try {
    ...
    application.statusHistory.push({
      ...
      changedBy: req.user._id,
      note: `Eligibility screened: ${isEligible ? "eligible" : "ineligible"}`,
    });
    await application.save();

    await createAuditLog({
      action: "eligibility_reviewed",
      performedBy: req.user._id,
      targetId: application._id,
      description: `Eligibility reviewed: ${isEligible ? "eligible" : "ineligible"}`,
      req,
    });

    return successResponse(res, {
      message: "Eligibility reviewed.",
      data: { application },
    });
  } catch (error) {
    next(error);
    ...
    const progress = await Evaluation.aggregate([
      {
        $group: {
          ...
          pending: { $sum: { $cond: ["$isSubmitted", 0, 1] } },
      ...
      { $unwind: "$judge" },
      {
        $project: {
          _id: 0,
          judgeId: "$_id",
          judgeName: { $concat: ["$judge.firstName", " ", "$judge.lastName"] },
          email: "$judge.email",
          submitted: 1,
          pending: 1,
  ...
  try {
    ...
    if (format === "pdf") {
      const lines = applications.map(
        (app) =>
          `${app.referenceNumber || "N/A"} | ${app.projectTitle} | ${app.status} | ${app.averageScore?.toFixed(1) || "0"}`,
      );
      const pdfBuffer = Buffer.from(
        buildSimplePdf("AI Awards Report", lines),
        "utf8",
      );
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="ai-awards-report.pdf"',
      );
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="ai-awards-report.csv"',
    );
    return res.send(csv);
  } catch (error) {
    next(error);
  ...
  try {
    ...
    await Promise.all(
      applications.map(async (app) => {
        ...
        app.awardCitation =
          app.awardCitation ||
          `${app.projectTitle} has been recognized as a finalist.`;
        await app.save();
    ...
    await Promise.all(
      applications.map(async (app, index) => {
        ...
        app.certificateNumber =
          app.certificateNumber ||
          `CERT-${String(Date.now()).slice(-6)}-${String(index + 1).padStart(2, "0")}`;
        app.certificateIssuedAt = new Date();
        app.awardCitation =
          app.awardCitation ||
          `${app.projectTitle} has been recognized as a winner.`;
        await app.save();
...
const generateCertificates = async (req, res, next) => {
  try {
    ...
    const generated = applications.map((app) => {
      app.certificateNumber =
        app.certificateNumber ||
        `CERT-${String(Date.now()).slice(-6)}-${String(applications.indexOf(app) + 1).padStart(2, "0")}`;
      app.certificateIssuedAt = new Date();
      app.awardCitation =
        app.awardCitation ||
        `${app.projectTitle} has been recognized for excellence.`;
      return app;
    });
    ...
    return successResponse(res, {
      ...
      data: {
        ...
        certificates: generated.map((app) => ({
          id: app._id,
          certificateNumber: app.certificateNumber,
          awardCitation: app.awardCitation,
        })),
      },
    });
  } catch (error) {
