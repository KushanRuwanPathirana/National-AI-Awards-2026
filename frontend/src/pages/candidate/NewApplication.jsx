...
const judgingCriteria = [
  [
    ...
    20,
    "What makes this technically or conceptually novel vs existing solutions?",
  ],
  [
    "measurableImpact",
    "Measurable Impact (Business/National)",
    25,
    "Quantified results: revenue impact, cost savings, users served, efficiency gains with supporting data/metrics.",
  ],
  [
    "technicalExcellence",
    "Technical Excellence",
    20,
    "Architecture overview, models/frameworks used, performance benchmarks.",
  ],
  [
    "responsibleAI",
    "Responsible AI & Governance",
    15,
    "Data privacy measures, bias mitigation, explainability, compliance steps taken.",
  ],
  [
    "scalabilitySustainability",
    "Scalability & Sustainability",
    10,
    "Plan/evidence for scaling beyond current deployment.",
  ],
  [
    "executionEvidence",
    "Execution & Evidence",
    10,
    "Team capability, timeline of delivery, proof points such as links, screenshots, or demo video.",
  ],
];
...
const hasApplicationDeadlinePassed = () =>
  Date.now() >= new Date(APPLICATION_DEADLINE_CLOSES_AT).getTime();
...
const WordHint = ({ value, max }) => {
  ...
  return (
    <p
      className={`mt-1 text-[11px] ${count > max ? "text-red-300" : "text-slate-500"}`}
    >
      {count}/{max} words
    </p>
  );
...
const FieldError = ({ message }) =>
  message ? (
    <p className="mt-1 text-[11px] font-medium text-red-300">{message}</p>
  ) : null;
...
const NewApplication = () => {
  ...
  useEffect(() => {
    setValue("registrationNumber", candidateRegistrationNumber, {
      shouldDirty: false,
    });
  ...
  useEffect(() => {
    ...
    const fetchDraft = async () => {
      ...
      try {
        const { data } =
          await applicationService.getApplicationById(draftQueryId);
        ...
        reset({
          ...defaults,
          organisationName:
            draft.organisationName || draft.organizationName || "",
          ...
          categoryEligibilityConfirmed:
            !!draft.categoryEligibilityConfirmed || draft.isEligible === true,
          ...
          innovationOriginality:
            draft.innovationOriginality || draft.innovationDetails || "",
          measurableImpact: draft.measurableImpact || draft.impactDetails || "",
          technicalExcellence:
            draft.technicalExcellence || draft.aiTechnologies || "",
        ...
        setCurrentStep(
          Math.min(
            Math.max(draft.completedStep || 0, 0),
            draftActiveStepsLength - 1,
          ),
        );
      } catch (e) {
  ...
  const buildPayload = (
    values,
    completedStep = Math.min(currentStep + 1, activeSteps.length),
  ) => {
    const payload = {
      ...
      eligibilityAnswers: [
        {
          question:
            "I confirm this entry meets the eligibility criteria for the selected category.",
  ...
  const ensureDraft = async (values = getValues()) => {
    ...
    const { data } = await applicationService.createDraft(
      buildPayload(values, currentStep),
    );
  ...
  const validateStep = (values) => {
    if (currentStep === 0) {
      ...
      if (missing)
        return { field: missing[0], message: `Please fill in ${missing[1]}.` };
    ...
    if (currentStep === 1) {
      if (
        ...
        return {
          field: "categoryId",
          message: "Please select a valid award category.",
        };
      }
      if (!values.categoryEligibilityConfirmed) {
        return {
          field: "categoryEligibilityConfirmed",
          message:
            "Please confirm this entry meets the selected category eligibility criteria.",
        };
      }
    }
    if (currentStep === 2) {
      ...
      if (missing)
        return { field: missing[0], message: `Please fill in ${missing[1]}.` };
      if (countWords(values.tagline) > 25)
        return {
          field: "tagline",
          message: "One-line summary must be 25 words or fewer.",
        };
      if (countWords(values.problemStatement) > 300)
        return {
          field: "problemStatement",
          message: "Problem statement must be 300 words or fewer.",
        };
      if (countWords(values.solution) > 500)
        return {
          field: "solution",
          message: "Solution description must be 500 words or fewer.",
        };
    }
    if (currentStep === 3) {
      ...
      if (missing)
        return {
          field: missing[0],
          message: `Please fill in ${missing[1]} evidence.`,
        };
    }
    if (currentStep === 5 && !values.nationalRelevance) {
      return {
        field: "nationalRelevance",
        message: "Please describe the Sri Lanka-specific national relevance.",
      };
    }
    if (currentStep === 6) {
      const required = [
        [
          "declarationAccepted",
          "Please confirm the accuracy of all information provided.",
        ],
        [
          "verificationConsent",
          "Please provide consent for verification if shortlisted.",
        ],
        ["promotionalConsent", "Please provide promotional consent."],
        [
          "submissionFeeAcknowledged",
          "Please acknowledge the submission fee requirements.",
        ],
      ];
      const missing = required.find(([field]) => !values[field]);
    ...
    if (currentStep === 7) {
      if (!isFreeCategory) {
        ...
        if (
          ...
          !values.onlinePaymentSimulated
        ) {
          return {
            field: "onlinePayment",
            message: "Please complete the online payment simulation.",
          };
        }
      }
    }
  ...
  const handleNext = async () => {
    if (applicationDeadlinePassed) {
      toast.error(
        `Applications can no longer be edited after the ${APPLICATION_DEADLINE_LABEL} deadline.`,
      );
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(e, "Failed to save application progress."),
      );
    }
  };
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      setFieldErrors({});
      const id = await ensureDraft(values);
      await applicationService.updateDraft(
        id,
        buildPayload(values, currentStep),
      );
  ...
  const handleFileUpload = async (e) => {
    if (applicationDeadlinePassed) {
      toast.error(
        `Documents can no longer be edited after the ${APPLICATION_DEADLINE_LABEL} deadline.`,
      );
      e.target.value = "";
      return;
    }
  ...
  const handlePaymentSlipUpload = async (e) => {
    if (applicationDeadlinePassed) {
      toast.error(
        `Slip can no longer be uploaded after the ${APPLICATION_DEADLINE_LABEL} deadline.`,
      );
      e.target.value = "";
      return;
    }
  ...
  const handlePaymentSlipDelete = async () => {
    if (applicationDeadlinePassed) {
      toast.error(
        `Slip can no longer be modified after the ${APPLICATION_DEADLINE_LABEL} deadline.`,
      );
      return;
    }
  ...
  // 1. මේ function එක වෙනස් කරන්න
  const handleSimulateCardPayment = async () => {
    try {
      setPaymentSimulating(true);

      // A. මුලින්ම දැනට කරපු ටික draft එකක් විදිහට DB එකේ සේව් කරගන්නවා
      const id = await ensureDraft();

      // B. Backend එකෙන් signed PayHere parameters ටික ඉල්ලගන්නවා
      ...
      // D. PayHere Checkout Modal එක open කරනවා 🚀
      window.payhere.startPayment(paymentConfig);
    } catch (e) {
      toast.error(
        getApiErrorMessage(e, "Failed to initialize PayHere gateway."),
      );
    } finally {
  ...
  const onSubmit = async () => {
    if (applicationDeadlinePassed) {
      toast.error(
        `Applications can no longer be submitted after the ${APPLICATION_DEADLINE_LABEL} deadline.`,
      );
      navigate("/dashboard?tab=drafts");
      return;
    }
    ...
    const requiredConsents =
      ...
      values.promotionalConsent &&
      values.submissionFeeAcknowledged &&
      values.termsAndPrivacyAccepted;
    if (!requiredConsents) {
      setFieldErrors({
        declarationAccepted: !values.declarationAccepted
        ...
        verificationConsent: !values.verificationConsent
          ...
          : "",
        promotionalConsent: !values.promotionalConsent
          ? "Please provide promotional consent."
          : "",
        submissionFeeAcknowledged: !values.submissionFeeAcknowledged
          ? "Please acknowledge the submission fee requirement."
          : "",
    ...
    try {
      ...
      await applicationService.updateDraft(
        id,
        buildPayload({ ...values, declarationAccepted: true }),
      );
  ...
  const fieldClass = (field, base = inputClass) =>
    `${base} ${fieldErrors[field] ? "!border-red-400 focus:!border-red-400 focus:!shadow-none" : ""}`;
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className="font-display font-black text-3xl text-white">
            {draftId
              ...
              : "National AI Awards 2026 Submission"}
          </h1>
          <p className="text-slate-400 text-sm mt-2">
            One submission per primary award category. Multiple categories
        ...
        {loadingDraft ? (
          ...
          <div className="glass-card p-8 sm:p-10 !hover:transform-none text-center">
            ...
            <p className="text-slate-400 text-sm mt-3 max-w-xl mx-auto leading-relaxed">
              The application deadline was {APPLICATION_DEADLINE_LABEL}. Drafts
              are now locked and can no longer be edited or submitted.
            </p>
            <Button
              variant="primary"
              className="mt-6 text-xs"
              onClick={() => navigate("/dashboard?tab=drafts")}
            >
              Back to Edit Draft
            </Button>
          </div>
              ...
              <div className="min-w-[720px] flex justify-between items-center relative">
                ...
                {activeSteps.map((step, idx) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] mt-2 font-medium uppercase tracking-wider ${idx === currentStep ? "text-accent-400" : "text-slate-500"}`}
                    >
                      {step.label}
                    </span>
                  </div>
                ...
                >
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <p className="text-slate-400 text-xs mt-1">
                          Provide the applicant details and authorised contact
                          for this entry.
                        </p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                          ...
                          <input
                            className={fieldClass("organisationName")}
                            {...register("organisationName")}
                          />
                          <FieldError message={fieldErrors.organisationName} />
                        </div>
                        <div>
                          ...
                          <p className="mt-1 text-[11px] text-slate-500">
                            Automatically assigned to your candidate account.
                          </p>
                        </div>
                        <div>
                          <FieldLabel required>Sector/Industry</FieldLabel>
                          <input
                            className={fieldClass("sectorIndustry")}
                            {...register("sectorIndustry")}
                          />
                          <FieldError message={fieldErrors.sectorIndustry} />
                        </div>
                        <div>
                          <FieldLabel required>Organisation size</FieldLabel>
                          <select
                            className={fieldClass("organisationSize")}
                        ...
                        <div>
                          ...
                          <input
                            className={fieldClass("primaryContactName")}
                            {...register("primaryContactName")}
                          />
                          <FieldError
                            message={fieldErrors.primaryContactName}
                          />
                        </div>
                        <div>
                          <FieldLabel required>Designation</FieldLabel>
                          <input
                            className={fieldClass("primaryContactDesignation")}
                            {...register("primaryContactDesignation")}
                          />
                          <FieldError
                            message={fieldErrors.primaryContactDesignation}
                          />
                        </div>
                        <div>
                          ...
                          />
                          <FieldError
                            message={fieldErrors.primaryContactEmail}
                          />
                        </div>
                        <div>
                          ...
                          />
                          <FieldError
                            message={fieldErrors.primaryContactPhone}
                          />
                        </div>
                        <div>
                  ...
                  {currentStep === 1 && (
                    <div className="space-y-6">
                      <div>
                        <h3 className="font-display font-bold text-white text-lg">
                          Section B: Award Category Selection
                        </h3>
                        <p className="text-slate-400 text-xs mt-1">
                          Select one primary award category for this submission.
                        </p>
                      </div>
                      <div>
                        ...
                        {categories.length === 0 && (
                          <p className="mt-2 text-xs text-amber-300">
                            No active award categories are available. Please ask
                            an administrator to seed or activate categories.
                          </p>
                        )}
                      </div>
                      {selectedCategory && (
                        <div className="rounded-xl bg-accent-500/5 border border-accent-500/20 p-4">
                          <h4 className="text-accent-400 text-xs font-bold uppercase tracking-wider">
                            Category Focus
                          </h4>
                      ...
                      <label
                        className={`flex gap-3 p-4 rounded-xl bg-white/5 border ${fieldErrors.categoryEligibilityConfirmed ? "border-red-400" : "border-white/5"}`}
                      >
                        <input
                          type="checkbox"
                          className="w-5 h-5 accent-accent-500 mt-0.5"
                          {...register("categoryEligibilityConfirmed")}
                        />
                        <span className="text-slate-300 text-sm leading-relaxed">
                          I confirm this entry meets the eligibility criteria
                          for the selected category.
                        </span>
                      </label>
                      <FieldError
                        message={fieldErrors.categoryEligibilityConfirmed}
                      />
                    </div>
                  )}
                  ...
                  {currentStep === 2 && (
                    <div className="space-y-6">
                      <div>
                        ...
                        <p className="text-slate-400 text-xs mt-1">
                          The core narrative used for screening and shortlist
                          announcements.
                        </p>
                      </div>
                      <div>
                        <FieldLabel required>Solution/Project name</FieldLabel>
                        <input
                          className={fieldClass("projectTitle")}
                          {...register("projectTitle")}
                        />
                        <FieldError message={fieldErrors.projectTitle} />
                      </div>
                      <div>
                        <FieldLabel required>One-line summary</FieldLabel>
                        <input
                          className={fieldClass("tagline")}
                          placeholder="25 words or fewer"
                          {...register("tagline")}
                        />
                        <WordHint value={watched.tagline} max={25} />
                      ...
                      <div>
                        ...
                        />
                        <WordHint value={watched.problemStatement} max={300} />
                        <FieldError message={fieldErrors.problemStatement} />
                      </div>
                      <div>
                        <FieldLabel required>Solution description</FieldLabel>
                        <textarea
                          className={fieldClass(
                            "solution",
                            "input-field resize-none min-h-[160px]",
                          )}
                          placeholder="How does it work, and what AI/ML techniques are used?"
                          {...register("solution")}
                        />
                        <WordHint value={watched.solution} max={500} />
                        <FieldError message={fieldErrors.solution} />
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        <div>
                          <FieldLabel required>Deployment status</FieldLabel>
                          <select
                            className={fieldClass("deploymentStatus")}
                        ...
                        <div>
                          <FieldLabel required>Date of launch</FieldLabel>
                          <input
                            type="date"
                            className={fieldClass("launchDate")}
                            {...register("launchDate")}
                          />
                          <FieldError message={fieldErrors.launchDate} />
                        </div>
                        <div>
                          <FieldLabel>Customer reference/revenue</FieldLabel>
                          <input
                            className={inputClass}
                            {...register("customerReferenceRevenue")}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  {currentStep === 3 && (
                    <div className="space-y-6">
                      <div>
                        ...
                        <p className="text-slate-400 text-xs mt-1">
                          Judges score directly against these weighted criteria.
                        </p>
                      </div>
                      {judgingCriteria.map(([key, label, weight, prompt]) => (
                        ...
                        >
                          ...
                          <textarea
                            className={fieldClass(key, textareaClass)}
                            {...register(key)}
                          />
                  ...
                  {currentStep === 4 && (
                    <div className="space-y-6">
                      <div>
                        ...
                        <p className="text-slate-400 text-xs mt-1">
                          Keep evidence concise and comparable across
                          applicants.
                        </p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                          ...
                          <input
                            className={inputClass}
                            placeholder="One live demo, product page, or video URL"
                            {...register("projectUrl")}
                          />
                        </div>
                        <div>
                          <FieldLabel>Optional demo video link</FieldLabel>
                          <input
                            className={inputClass}
                            placeholder="YouTube/Vimeo, 2-3 minutes"
                            {...register("demoVideoUrl")}
                          />
                        </div>
                        <div>
                          <FieldLabel>
                            Testimonial/client reference 1
                          </FieldLabel>
                          <input
                            className={inputClass}
                            {...register("testimonialOne")}
                          />
                        </div>
                        <div>
                          <FieldLabel>
                            Testimonial/client reference 2
                          </FieldLabel>
                          <input
                            className={inputClass}
                            {...register("testimonialTwo")}
                          />
                        </div>
                      </div>
                      <div className="border border-dashed border-white/10 rounded-2xl p-8 text-center bg-white/5 relative">
                        <input
                          ...
                          disabled={uploading || uploadedFiles.length >= 2}
                        />
                        <div className="flex flex-col items-center">
                          ...
                          <h4 className="font-display font-semibold text-white text-sm mb-1">
                            {uploading
                              ? "Uploading..."
                              : "Upload up to 2 PDF documents"}
                          </h4>
                          <p className="text-slate-500 text-xs">
                            Suggested cap: 5 pages each, max 10MB per file.
                          </p>
                        </div>
                      </div>
                      {uploadedFiles.length > 0 && (
                        <div className="space-y-2">
                          ...
                          {uploadedFiles.map((file) => (
                            <div
                              key={file._id}
                              className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5 text-xs text-slate-300"
                            >
                  ...
                  {currentStep === 5 && (
                    <div className="space-y-6">
                      <div>
                        <h3 className="font-display font-bold text-white text-lg">
                          Section F: National Relevance
                        </h3>
                        <p className="text-slate-400 text-xs mt-1">
                          Describe the Sri Lanka-specific need, local language
                          support, or national development relevance.
                        </p>
                      </div>
                      <textarea
                        className={fieldClass(
                          "nationalRelevance",
                          "input-field resize-none min-h-[180px]",
                        )}
                        placeholder="How does this solution address Sinhala/Tamil support, local market needs, or national development goals?"
                        {...register("nationalRelevance")}
                      />
                  ...
                  {currentStep === 6 && (
                    <div className="space-y-6">
                      <div>
                        <h3 className="font-display font-bold text-white text-lg">
                          Section G: Declarations & Consent
                        </h3>
                        <p className="text-slate-400 text-xs mt-1">
                          Confirm authority, accuracy, verification consent, and
                          conflict disclosure before final submission.
                        </p>
                      </div>
                      <div className="rounded-2xl bg-white/5 border border-white/5 p-5 space-y-4">
                        {[
                          [
                            "declarationAccepted",
                            "I confirm the accuracy of all information provided in this application.",
                          ],
                          [
                            "verificationConsent",
                            "I consent to a site visit or demo verification if shortlisted.",
                          ],
                          [
                            "promotionalConsent",
                            "I consent to be featured in promotional materials if shortlisted or winning.",
                          ],
                          [
                            "submissionFeeAcknowledged",
                            "I acknowledge the submission fee requirements for this entry.",
                          ],
                        ].map(([field, label]) => (
                          <div key={field}>
                            <label
                              className={`flex gap-3 rounded-xl border p-3 ${fieldErrors[field] ? "border-red-400 bg-red-500/5" : "border-transparent"}`}
                            >
                              <input
                                type="checkbox"
                                className="w-5 h-5 accent-accent-500 mt-0.5"
                        ...
                        <div>
                          <label
                            className={`flex gap-3 rounded-xl border p-3 ${fieldErrors.termsAndPrivacyAccepted ? "border-red-400 bg-red-500/5" : "border-transparent"}`}
                          >
                            <input
                              type="checkbox"
                              className="w-5 h-5 accent-accent-500 mt-0.5"
                            ...
                            <span className="text-slate-300 text-sm leading-relaxed">
                              ...
                              <Link
                                ...
                                className="text-accent-400 hover:text-accent-300 underline font-medium"
                              >
                                Terms of Use
                              </Link>{" "}
                              and{" "}
                          ...
                          <FieldError
                            message={fieldErrors.termsAndPrivacyAccepted}
                          />
                        </div>
                      </div>
                      <div>
                        <FieldLabel>Conflict of interest disclosure</FieldLabel>
                        <textarea
                          className={textareaClass}
                          placeholder="Declare any judge affiliation, committee relationship, or write N/A."
                          {...register("conflictDisclosure")}
                        />
                      </div>
                      <div className="rounded-xl bg-accent-500/5 border border-accent-500/20 p-4 flex gap-3">
                        ...
                        <p className="text-slate-300 text-xs leading-relaxed">
                          Once submitted, this candidate application enters the
                          review workflow and cannot be modified from the
                          portal.
                        </p>
                      </div>
                    </div>
                  ...
                  {currentStep === 7 && !isFreeCategory && (
                    <div className="space-y-6">
                      ...
                      <div>
                        <h3 className="font-display font-bold text-white text-lg">
                          Section H: Registration Fee & Payment
                        </h3>
                        <p className="text-slate-400 text-xs mt-1">
                          Application fee is LKR 25,000. Choose your preferred
                          payment method.
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        >
                          <RiBankLine
                            className={`text-2xl mt-0.5 ${watched.paymentMethod === "transfer" ? "text-accent-400" : "text-slate-400"}`}
                          />
                          <div>
                        ...
                        >
                          <RiBankCardLine
                            className={`text-2xl mt-0.5 ${watched.paymentMethod === "online" ? "text-accent-400" : "text-slate-400"}`}
                          />
                          <div>
                            <h4 className="text-white font-bold text-sm">
                              Online Card Payment
                            ...
                            <p className="text-slate-400 text-xs mt-1">
                              Pay securely via Credit/Debit card (mocked
                              authorization flow).
                            </p>
                          </div>
                        </button>
                        ...
                        <div className="space-y-5 rounded-2xl bg-white/[0.02] border border-white/5 p-6 animate-fadeIn">
                          <div>
                            ...
                            <p className="text-slate-400 text-xs mt-1">
                              ...
                              and upload your payment slip below. Make sure to
                              mention your registration number{" "}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                            ...
                            <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                              ...
                              <strong className="text-white text-sm block mt-1">
                                National AI Awards Association
                              </strong>
                            </div>
                            <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                              <span className="text-slate-500 font-medium block">
                              ...
                              <strong className="text-white text-sm font-mono block mt-1">
                                0123-456789-012
                              </strong>
                            </div>
                            <div className="p-4 rounded-xl bg-white/5 border border-white/5">
                      ...
                      {watched.paymentMethod === "online" && (
                        <div className="rounded-2xl bg-white/[0.02] border border-white/5 p-6 animate-fadeIn text-center space-y-4">
                          <div>
                            <h4 className="text-white font-bold text-sm mb-1">
                              Pay Online via PayHere Gateway
                            </h4>
                            <p className="text-slate-400 text-xs">
                              Click below to securely pay the LKR 25,000 award
                              registration fee.
                            </p>
                          </div>

                          {watched.onlinePaymentSimulated ? (
                            <div className="p-6 rounded-xl bg-emerald-500/15 border border-emerald-500/20 max-w-md mx-auto space-y-3">
                            ...
                            <div className="max-w-xs mx-auto pt-2">
                              <Button
                                type="button"
                                variant="gold"
                                className="w-full text-xs flex justify-center items-center gap-2"
                                onClick={handleSimulateCardPayment}
                                loading={paymentSimulating}
                              >
                                Proceed to Pay LKR 25,000
                              </Button>
                            </div>
                          )}
                        </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
                  ...
                  {currentStep < activeSteps.length - 1 ? (
                    <Button
                      ...
                      onClick={handleNext}
                    >
                      Save & Continue <RiArrowRightLine />
                    </Button>
                  ) : (
                    <Button
                      variant="gold"
